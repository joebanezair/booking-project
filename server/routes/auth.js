...
async function publicUser(user) {
  const business = user.role === "business"
    ? await Business.findOne({ owner: user._id }).select("_id name category").lean()
    : null;
  return {
    id: user._id,
    name: user.name,
    ...
    business: business ? { id: business._id, name: business.name, category: business.category } : null
  };
}

function slug(value) {
  return String(value || "").toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 30) || "business";
}
...
router.post("/register", async (req, res) => {
  try {
    await createAccount(req, res, "business");
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Unable to create business account." });
  }
});

router.post("/register/admin/:inviteKey", async (req, res) => {
  try {
    if (!validInviteKey(req.params.inviteKey, process.env.ADMIN_REGISTRATION_KEY)) {
      return res.status(403).json({ message: "This administrator registration invitation is invalid or unavailable." });
    }
    await createAccount(req, res, "admin");
  } catch (error) {
    console.error(error);
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!["business", "admin"].includes(user.role)) {
      return res.status(403).json({ message: "This legacy customer account is no longer used. Visitors can book services as guests." });
    }
