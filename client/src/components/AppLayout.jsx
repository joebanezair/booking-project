    ...
    <aside className="sidebar">
      <div>
        ...
        <nav className="sidebar-nav" aria-label="Dashboard navigation">
          {navigation.filter(item => (!item.adminOnly || user?.role === "admin") && (!item.providerOnly || ["admin", "business"].includes(user?.role))).map(({ to, label, icon: Icon, end }) => <NavLink to={to} end={end} key={to}><Icon aria-hidden="true" /><span>{label}</span>{label==="Notifications"&&unread>0&&<b className="nav-badge">{unread>99?"99+":unread}</b>}</NavLink>)}
        </nav>
      </div>
      <div><small className="sidebar-email">{user?.role === "admin" ? "Admin" : user?.role === "business" ? "Business" : "Customer"} · {user?.email}</small><button className="secondary signout-button" onClick={onLogout}><FiLogOut aria-hidden="true" /><span>Sign out</span></button></div>
    </aside>
