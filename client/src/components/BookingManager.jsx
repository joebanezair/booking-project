...
export default function BookingManager({user}){
  ...
  useEffect(()=>{
    Promise.all([api.bookings.list(),api.content.list(),api.bookings.customers()]).then(([bookings,content,people])=>{setItems(bookings);setServices(content);setCustomers(people);}).catch(e=>setError(e.message));
