...
const sections = [
  { title: "Services", text: "Create and manage public, private, or draft services.", to: "/dashboard/services", action: "Manage services" },
...
export default function DashboardPage({ user, onLogout }) {
  return <AppLayout user={user} onLogout={onLogout}>
    <header className="topbar"><div><p className="eyebrow">DASHBOARD</p><h1>Welcome back, {user.name.split(" ")[0]}</h1><p className="muted">Choose an area to manage. Each feature has its own page.</p></div><Link className="primary-button button-link" to="/dashboard/services/new">Post a service</Link></header>
